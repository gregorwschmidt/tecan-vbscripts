Option Explicit
Dim strExcelPath, oXL, oXLBook, oXLSheet, bReadOnly, objShell, intExcelRow, intExcelColumn, strExcelInputValue, strSheetName, intExcelNoRows, intExcelNoColumns, i, j

' Read values from evoware
strExcelPath = cstr(Evoware.GetStringVariable("VBScript_SetExcelSheetMultipleCellValueString_FileNamePath"))
intExcelRow = cint(Evoware.GetDoubleVariable("VBScript_SetExcelSheetMultipleCellValueString_Row"))
intExcelColumn = cint(Evoware.GetDoubleVariable("VBScript_SetExcelSheetMultipleCellValueString_Column"))
intExcelNoRows = cint(Evoware.GetDoubleVariable("VBScript_SetExcelSheetMultipleCellValueString_NoRows"))
intExcelNoColumns = cint(Evoware.GetDoubleVariable("VBScript_SetExcelSheetMultipleCellValueString_NoColumns"))
strExcelInputValue = cstr(Evoware.GetStringVariable("VBScript_SetExcelSheetMultipleCellValueString_String"))
strSheetName = cstr(Evoware.GetStringVariable("VBScript_SetExcelSheetMultipleCellValueString_SheetName"))
...
For i = 0 to intExcelNoColumns-1
    ...
    oXLSheet.Cells(intExcelRow+j, intExcelColumn+i).Value = strExcelInputValue
	Next
Next
'Save and quit.
