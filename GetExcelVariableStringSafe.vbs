...
Dim oXL, oXLBook, oXLSheet, objShell, strExcelOutputValue, fso, strExcelFilePath, Temp, strFileExtension, strExcelCopyFileNamePath, dateTime, intReturnCode

' Read values from evoware
strExcelFileNamePath = cstr(Evoware.GetStringVariable("VBScript_GetExcelVariableStringSafe_FileNamePath"))
strExcelVariableName = cstr(Evoware.GetStringVariable("VBScript_GetExcelVariableStringSafe_VariableName"))
...
If fso.FileExists(strExcelFileNamePath) Then
	...
	oXLBook.Activate
	'Retrieve value
	strExcelOutputValue = oXLBook.Worksheets(1).Range(strExcelVariableName).Value 

	'Handover value to EVOware
	Evoware.SetStringVariable strExcelVariableName, cstr(strExcelOutputValue)
...
Evoware.SetDoubleVariable "VBScript_GetExcelVariableStringSafe_ReturnCode", intReturnCode
