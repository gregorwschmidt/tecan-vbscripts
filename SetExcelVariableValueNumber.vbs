Option Explicit
Dim strExcelPath, oXL, oXLBook, oXLSheet, bReadOnly, objShell, floExcelInputValue, strExcelVariableName

' Read values from evoware
strExcelPath = cstr(Evoware.GetStringVariable("VBScript_SetExcelVariableValueNumber_FileNamePath"))
strExcelVariableName = cstr(Evoware.GetStringVariable("VBScript_SetExcelVariableValueNumber_VariableName"))
floExcelInputValue = cdbl(Evoware.GetDoubleVariable("VBScript_SetExcelVariableValueNumber_Number"))
...
oXLSheet.Range(strExcelVariableName).Value = floExcelInputValue
'Save and quit.
...
Set floExcelInputValue = Nothing
